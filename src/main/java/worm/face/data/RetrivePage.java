...
public class RetrivePage {
	 ...
	 * @throws Exception
	 */
	public static int getDouBanList(String surl) throws Exception {
	...
	public static void skipPage(String surl) throws Exception {
		...
		// (共210条)
		int total = Integer.parseInt(doc.getElementsByClass("count").text()
				.replace("(共", "").replace("条)", ""));
		int end = Integer.parseInt(thispage.text()); // 获取当前页码中的数字元素（String类型），并转化为int类型
		if (end == 1) {
			pageSize = getDouBanList(surl);
			System.out.println("thispage: " + 1 + ">>>>>>>>>>>> total: "
					+ total + ">>>>>>>>>>>> pageSize: " + pageSize);
		}
		getDouBanList(http); // 爬取下一页面
		System.out.println("thispage: " + end + ">>>>>>>>>>>> total: " + total
				+ ">>>>>>>>>>>> pageSize: " + pageSize);
		if (end < total / pageSize) {
...
//		String strURL = "https://movie.douban.com/tag/%E5%96%9C%E5%89%A7";
		try {
